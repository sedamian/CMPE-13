...
int main(void) {
    ...
    while (1) {
        ...
        if (eData.event == TRUE) { // if true, enter 
            LEDS_SET(ball); //sets leds
            if (ball == 0x80) { // checks edge
                dir = RIGHT; // changes direction
            } else if (ball == 0x01) { // checks other edge
                dir = LEFT; // change direction
            }
            if (dir == LEFT) {
                ball = ball << 1; // bitshift left. if dir is left
            } else {
                ball = ball >> 1; // bitsh ift right
            }
        }
        eData.event = FALSE; // resets to false
...
void __ISR(_TIMER_1_VECTOR, IPL4AUTO) Timer1Handler(void) {
    ...
    eData.value++;
    if (eData.value > SWITCH_STATES()) { // checks struct value
        eData.event = TRUE;
        eData.value = 0;
    }
