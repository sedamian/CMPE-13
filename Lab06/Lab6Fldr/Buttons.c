...
uint8_t ButtonsCheckEvents(void) {
    ...
    if ((0x01 == BUTTON_STATES())&&(curr == 0x00)) { // goes into first state
        curr = 0x01; // places into next
        return BUTTON_EVENT_1UP;



    }
    if ((0x00 == BUTTON_STATES())&&(curr == 0x01)) { //goes into next state
        curr = 0x00;
        return BUTTON_EVENT_1DOWN;
    }
    else {
        return BUTTON_EVENT_NONE; // return if no change
    }
    notfirst = 5;
